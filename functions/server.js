...
exports.handler = async (event, context) => {
    try {
        ...
        let buyersArray = Array.isArray(testing_media_buyers) ? testing_media_buyers : [testing_media_buyers];
        ...
        const response = await notion.pages.create({
            ...
            properties: {
                ...
                "Project Name(mo.day-category,name,type)": { title: [{ text: { content: project_name } }] },
                ...
                "Headline (on platform)": { rich_text: [{ text: { content: headline_on_platform } }] },
                ...
                ...(visual_headline && { "Visual Headline": { rich_text: [{ text: { content: visual_headline } }] } }),
                "V Headlines": { number: parseInt(v_headlines) },
                ...(cc_scripts !== undefined && cc_scripts !== null && cc_scripts !== '' && !isNaN(cc_scripts) && { "CC/Scripts": { number: parseInt(cc_scripts) } }),
                ...
                "Testing Media Buyers": { multi_select: buyersArray.map(buyer => ({ name: buyer })) },
            },
        });

        return {
            statusCode: 200,
            body: JSON.stringify({ message: 'Form data successfully submitted to Notion!' })
        };
    } catch (error) {
        console.error(error);
        return {
            statusCode: 500,
            body: JSON.stringify({ message: 'There was an error submitting the form.', error: error.message })
        };
    }
};